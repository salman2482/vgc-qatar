...
protected function previewDrill(Request $request,$id)
 {
  $user = Auth::user();
...
//$html=view('user.drill.preview.index',compact(['user','drill']))->render();
...
if ($request->isMethod('post')) {
        ...
        
    {
        ...
        if(!empty($request->qmins) && !empty($request->qnote))  //Saving Quick Notes

        {
         $arr1=array_values($request->qmins);
        $arr2=array_values($request->qnote);

        $i=0;
        while(isset($arr1[$i]))
        {

        DB::table('plan_quick_notes')->insert(
            ['user_id' =>$user->id,'plan_id' =>$data->id,
             'notes' =>$arr2[$i],'mins' =>$arr1[$i]
            ]
        );
        ...
        if(!empty($request->dmins) && !empty($request->drill_id))   //Saving Quick Drills

        {
         $arr1=array_values($request->dmins);
        $arr2=array_values($request->drill_id);

        $i=0;
        while(isset($arr1[$i]))
        {

        DB::table('plan_drills')->insert(
            ['user_id' =>$user->id,'plan_id' =>$data->id,
             'drill_id' =>$arr2[$i],'mins' =>$arr1[$i]
            ]
        );
...
if ($request->isMethod('post')) {
        ...
        
    {
        ...
        {
            $current_notes=  DB::table('plan_quick_notes')->where('plan_id',$request->id)->where('user_id',$user->id)->get();
            //dd($current_notes);
...
$quicknotes=DB::table('plan_quick_notes')->where('plan_id',$plan->id)->get();
$quickdrills=DB::table('plan_drills')->where('plan_id',$plan->id)->get();

$teams=Team::where('creator_id',$user->id)->get();

return view('user.viewplan',compact(['user','taggedDrills','tags','plan','quicknotes','teams','quickdrills']));
    ...
    if($id=$request->plan_id)
     {
      ...
      $delete=DB::table('plan_quick_notes')->where('user_id', $user->id)->where('plan_id', $id)->delete();
...
protected function deletePlanNote(Request $request,$id)
{
    ...
    if($id=$request->id) {
      ...
      $delete=DB::table('plan_quick_notes')->where('user_id', $user->id)->where('id', $id)->delete();
    if($delete) 
    {
...
protected function deletePlanDrill(Request $request,$id)
{
    ...
    if($id=$request->id) {
      ...
      $delete=DB::table('plan_drills')->where('user_id', $user->id)->where('id', $id)->delete();
    if($delete) 
    {
