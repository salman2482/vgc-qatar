            ...
            foreach ($emails as $email) {
                ...
                if ($email->emailqueue_pdf_resource_type == 'invoice') {
                    if (!$payload = $invoicegenerator->generate($bill_id)) {
                        Log::error("the invoice could not be generated", ['process' => '[Email Bill Cronjob]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'bill_id' => $bill_id]);
                    }
                }
