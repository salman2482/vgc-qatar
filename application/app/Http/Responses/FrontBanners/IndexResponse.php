...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the FrontBanners
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/frontbanner/components/table/ajax';
                $dom_container = '#frontbanners-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/frontbanner/components/table/table';
                $dom_container = '#frontbanners-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/frontbanner/tabswrapper';
            ...
            if ($frontbanners->currentPage() < $frontbanners->lastPage()) {
                $url = loadMoreButtonUrl($frontbanners->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'frontbanners'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.frontbanners')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-frontbanners',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($frontbanners->currentPage() + 1, request('source'));
            $page['loading_target'] = 'frontbanners-td-container';
            $page['visibility_show_load_more'] = ($frontbanners->currentPage() < $frontbanners->lastPage()) ? true : false;
            return view('pages/frontbanner/wrapper', compact('page', 'frontbanners'))->render();
        }
