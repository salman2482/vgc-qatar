    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/bookings/components/table/ajax';
                    $dom_container = '#bookings-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/bookings/components/table/table';
                    $dom_container = '#bookings-table-wrapper';
            ...
            if ($bookings->currentPage() < $bookings->lastPage()) {
                $url = loadMoreButtonUrl($bookings->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'bookings'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($bookings->currentPage() + 1, request('source'));
            $page['loading_target'] = 'bookings-td-container';
            $page['visibility_show_load_more'] = ($bookings->currentPage() < $bookings->lastPage()) ? true : false;
            return view('pages/bookings/wrapper', compact('page', 'bookings'))->render();
        }
    }
}
