...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the vendorpos
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/vendorpo/components/table/ajax';
                $dom_container = '#vendorpos-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/vendorpo/components/table/table';
                $dom_container = '#vendorpos-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/vendorpo/tabswrapper';
            ...
            if ($vendorpos->currentPage() < $vendorpos->lastPage()) {
                $url = loadMoreButtonUrl($vendorpos->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'vendorpos'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.vendorpos')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($vendorpos->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vendorpos-td-container';
            $page['visibility_show_load_more'] = ($vendorpos->currentPage() < $vendorpos->lastPage()) ? true : false;
            return view('pages/vendorpo/wrapper', compact('page', 'vendorpos'))->render();
        }
