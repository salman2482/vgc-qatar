...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/property/components/table/ajax';
                $dom_container = '#properties-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/property/components/table/table';
                $dom_container = '#properties-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/property/tabswrapper';
            ...
            $html = view($template, compact('page', 'properties'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($properties->currentPage() + 1, request('source'));
            $page['loading_target'] = 'properties-td-container';
            $page['visibility_show_load_more'] = ($properties->currentPage() < $properties->lastPage()) ? true : false;
            return view('pages/property/wrapper', compact('page', 'properties'))->render();
        }
