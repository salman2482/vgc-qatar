...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the SubProducts
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/subproduct/components/table/ajax';
                $dom_container = '#subproducts-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/subproduct/components/table/table';
                $dom_container = '#subproducts-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/subproduct/tabswrapper';
            ...
            if ($subproducts->currentPage() < $subproducts->lastPage()) {
                $url = loadMoreButtonUrl($subproducts->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'subproducts'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.subproducts')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-subproducts',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($subproducts->currentPage() + 1, request('source'));
            $page['loading_target'] = 'subproducts-td-container';
            $page['visibility_show_load_more'] = ($subproducts->currentPage() < $subproducts->lastPage()) ? true : false;
            return view('pages/subproduct/wrapper', compact('page', 'subproducts'))->render();
        }
