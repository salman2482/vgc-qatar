...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the vusers
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/frontusers/components/table/ajax';
                    $dom_container = '#frontusers-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/frontusers/components/table/table';
                    $dom_container = '#frontusers-table-wrapper';
                    $dom_action = 'replace-with';
                    break;

                    //template and dom - for ajax initial loading
                default:
                    $template = 'pages/frontusers/tabswrapper';
            ...
            if ($frontusers->currentPage() < $frontusers->lastPage()) {
                $url = loadMoreButtonUrl($frontusers->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'frontusers'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($frontusers->currentPage() + 1, request('source'));
            $page['loading_target'] = 'frontusers-td-container';
            $page['visibility_show_load_more'] = ($frontusers->currentPage() < $frontusers->lastPage()) ? true : false;
            return view('pages/frontusers/wrapper', compact('page', 'frontusers'))->render();
        }
    }
}
