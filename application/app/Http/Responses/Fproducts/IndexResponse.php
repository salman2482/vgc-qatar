...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the fproducts
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/fproduct/components/table/ajax';
                $dom_container = '#fproducts-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/fproduct/components/table/table';
                $dom_container = '#fproducts-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/fproduct/tabswrapper';
            ...
            if ($fproducts->currentPage() < $fproducts->lastPage()) {
                $url = loadMoreButtonUrl($fproducts->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'fproducts'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($fproducts->currentPage() + 1, request('source'));
            $page['loading_target'] = 'fproducts-td-container';
            $page['visibility_show_load_more'] = ($fproducts->currentPage() < $fproducts->lastPage()) ? true : false;
            return view('pages/fproduct/wrapper', compact('page', 'fproducts'))->render();
        }
    }
