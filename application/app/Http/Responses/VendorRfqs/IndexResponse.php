...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the vendorrfq
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/vendorrfq/components/table/ajax';
                $dom_container = '#vendorrfqs-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/vendorrfq/components/table/table';
                $dom_container = '#vendorrfqs-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/vendorrfq/tabswrapper';
            ...
            if ($vendorrfqs->currentPage() < $vendorrfqs->lastPage()) {
                $url = loadMoreButtonUrl($vendorrfqs->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'vendorrfqs'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.vendorrfqs')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($vendorrfqs->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vendorrfqs-td-container';
            $page['visibility_show_load_more'] = ($vendorrfqs->currentPage() < $vendorrfqs->lastPage()) ? true : false;
            return view('pages/vendorrfq/wrapper', compact('page', 'vendorrfqs'))->render();
        }
