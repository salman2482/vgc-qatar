...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/ritem/components/table/ajax';
                $dom_container = '#ritems-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/ritem/components/table/table';
                $dom_container = '#ritems-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/ritem/tabswrapper';
            ...
            if ($ritems->currentPage() < $ritems->lastPage()) {
                $url = loadMoreButtonUrl($ritems->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'ritems'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($ritems->currentPage() + 1, request('source'));
            $page['loading_target'] = 'ritems-td-container';
            $page['visibility_show_load_more'] = ($ritems->currentPage() < $ritems->lastPage()) ? true : false;
            return view('pages/ritem/wrapper', compact('page', 'ritems'))->render();
        }
