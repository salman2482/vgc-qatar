...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the CorporateServices
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/corporateservice/components/table/ajax';
                $dom_container = '#corporateservices-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/corporateservice/components/table/table';
                $dom_container = '#corporateservices-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/corporateservice/tabswrapper';
            ...
            if ($corporateservices->currentPage() < $corporateservices->lastPage()) {
                $url = loadMoreButtonUrl($corporateservices->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'corporateservices'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.corporateservices')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-corporateservices',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($corporateservices->currentPage() + 1, request('source'));
            $page['loading_target'] = 'corporateservices-td-container';
            $page['visibility_show_load_more'] = ($corporateservices->currentPage() < $corporateservices->lastPage()) ? true : false;
            return view('pages/corporateservice/wrapper', compact('page', 'corporateservices'))->render();
        }
