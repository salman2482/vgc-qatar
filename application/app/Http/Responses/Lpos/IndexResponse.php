...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/lpo/components/table/ajax';
                $dom_container = '#lpos-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/lpo/components/table/table';
                $dom_container = '#lpos-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/lpo/tabswrapper';
            ...
            if ($lpos->currentPage() < $lpos->lastPage()) {
                $url = loadMoreButtonUrl($lpos->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'lpos'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($lpos->currentPage() + 1, request('source'));
            $page['loading_target'] = 'lpos-td-container';
            $page['visibility_show_load_more'] = ($lpos->currentPage() < $lpos->lastPage()) ? true : false;
            return view('pages/lpo/wrapper', compact('page', 'lpos'))->render();
        }
