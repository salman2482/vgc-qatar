...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the CareersApply
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/careerapply/components/table/ajax';
                $dom_container = '#careersapply-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/careerapply/components/table/table';
                $dom_container = '#careersapply-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/careerapply/tabswrapper';
            ...
            if ($careersapply->currentPage() < $careersapply->lastPage()) {
                $url = loadMoreButtonUrl($careersapply->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'careersapply'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.careersapply')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-careersapply',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($careersapply->currentPage() + 1, request('source'));
            $page['loading_target'] = 'careersapply-td-container';
            $page['visibility_show_load_more'] = ($careersapply->currentPage() < $careersapply->lastPage()) ? true : false;
            return view('pages/careerapply/wrapper', compact('page', 'careersapply'))->render();
        }
