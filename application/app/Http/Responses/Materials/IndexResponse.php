...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/material/components/table/ajax';
                $dom_container = '#materials-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/material/components/table/table';
                $dom_container = '#materials-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/material/tabswrapper';
            ...
            if ($materials->currentPage() < $materials->lastPage()) {
                $url = loadMoreButtonUrl($materials->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'materials'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($materials->currentPage() + 1, request('source'));
            $page['loading_target'] = 'materials-td-container';
            $page['visibility_show_load_more'] = ($materials->currentPage() < $materials->lastPage()) ? true : false;
            return view('pages/material/wrapper', compact('page', 'materials'))->render();
        }
