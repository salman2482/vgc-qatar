...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the SubCorporateServices
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/subcorporateservice/components/table/ajax';
                $dom_container = '#subcorporateservices-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/subcorporateservice/components/table/table';
                $dom_container = '#subcorporateservices-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/subcorporateservice/tabswrapper';
            ...
            if ($subcorporateservices->currentPage() < $subcorporateservices->lastPage()) {
                $url = loadMoreButtonUrl($subcorporateservices->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'subcorporateservices'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.subcorporateservices')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-subcorporateservices',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($subcorporateservices->currentPage() + 1, request('source'));
            $page['loading_target'] = 'subcorporateservices-td-container';
            $page['visibility_show_load_more'] = ($subcorporateservices->currentPage() < $subcorporateservices->lastPage()) ? true : false;
            return view('pages/subcorporateservice/wrapper', compact('page', 'subcorporateservices'))->render();
        }
