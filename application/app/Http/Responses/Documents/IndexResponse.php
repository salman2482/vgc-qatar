...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the documents
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/document/components/table/ajax';
                $dom_container = '#documents-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/document/components/table/table';
                $dom_container = '#documents-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/document/tabswrapper';
            ...
            if ($documents->currentPage() < $documents->lastPage()) {
                $url = loadMoreButtonUrl($documents->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'documents'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.documents')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($documents->currentPage() + 1, request('source'));
            $page['loading_target'] = 'documents-td-container';
            $page['visibility_show_load_more'] = ($documents->currentPage() < $documents->lastPage()) ? true : false;
            return view('pages/document/wrapper', compact('page', 'documents'))->render();
        }
