...
namespace App\Http\Responses\VUsers;
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/vusers/components/table/ajax';
                $dom_container = '#vusers-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/vusers/components/table/table';
                $dom_container = '#vusers-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/vusers/tabswrapper';
            ...
            if ($vusers->currentPage() < $vusers->lastPage()) {
                $url = loadMoreButtonUrl($vusers->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'vusers'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($vusers->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vusers-td-container';
            $page['visibility_show_load_more'] = ($vusers->currentPage() < $vusers->lastPage()) ? true : false;
            return view('pages/vusers/wrapper', compact('page', 'vusers'))->render();
        }
    }
