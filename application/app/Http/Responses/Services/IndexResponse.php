...
namespace App\Http\Responses\Services;
    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/service/components/table/ajax';
                    $dom_container = '#services-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/service/components/table/table';
                    $dom_container = '#services-table-wrapper';
                    $dom_action = 'replace-with';
                    break;

                    //template and dom - for ajax initial loading
                default:
                    $template = 'pages/service/tabswrapper';
            ...
            if ($services->currentPage() < $services->lastPage()) {
                $url = loadMoreButtonUrl($services->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'services'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($services->currentPage() + 1, request('source'));
            $page['loading_target'] = 'services-td-container';
            $page['visibility_show_load_more'] = ($services->currentPage() < $services->lastPage()) ? true : false;
            return view('pages/service/wrapper', compact('page', 'services'))->render();
        }
    }
}
