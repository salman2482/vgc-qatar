...
namespace App\Http\Responses\MailLists;
    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/mailist/components/table/ajax';
                    $dom_container = '#mails-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/mailist/components/table/table';
                    $dom_container = '#mails-table-wrapper';
                    $dom_action = 'replace-with';
                    break;

                    //template and dom - for ajax initial loading
                default:
                    $template = 'pages/mailist/tabswrapper';
            ...
            if ($mails->currentPage() < $mails->lastPage()) {
                $url = loadMoreButtonUrl($mails->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'mails'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($mails->currentPage() + 1, request('source'));
            $page['loading_target'] = 'mails-td-container';
            $page['visibility_show_load_more'] = ($mails->currentPage() < $mails->lastPage()) ? true : false;
            return view('pages/mailist/wrapper', compact('page', 'mails'))->render();
        }
    }
}
