...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the FrontCareers
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/frontcareer/components/table/ajax';
                $dom_container = '#frontcareers-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/frontcareer/components/table/table';
                $dom_container = '#frontcareers-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/frontcareer/tabswrapper';
            ...
            if ($frontcareers->currentPage() < $frontcareers->lastPage()) {
                $url = loadMoreButtonUrl($frontcareers->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'frontcareers'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.frontcareers')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-frontcareers',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($frontcareers->currentPage() + 1, request('source'));
            $page['loading_target'] = 'frontcareers-td-container';
            $page['visibility_show_load_more'] = ($frontcareers->currentPage() < $frontcareers->lastPage()) ? true : false;
            return view('pages/frontcareer/wrapper', compact('page', 'frontcareers'))->render();
        }
