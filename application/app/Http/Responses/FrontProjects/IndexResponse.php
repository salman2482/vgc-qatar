...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the FrontProjects
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/frontproject/components/table/ajax';
                $dom_container = '#frontprojects-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/frontproject/components/table/table';
                $dom_container = '#frontprojects-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/frontproject/tabswrapper';
            ...
            if ($frontprojects->currentPage() < $frontprojects->lastPage()) {
                $url = loadMoreButtonUrl($frontprojects->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'frontprojects'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.frontprojects')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-frontprojects',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($frontprojects->currentPage() + 1, request('source'));
            $page['loading_target'] = 'frontprojects-td-container';
            $page['visibility_show_load_more'] = ($frontprojects->currentPage() < $frontprojects->lastPage()) ? true : false;
            return view('pages/frontproject/wrapper', compact('page', 'frontprojects'))->render();
        }
