...
class RoleRepository {
    ...
    public function create() {
        ...
        $role->role_rfms = (in_array(request('role_rfms'), $valid)) ? request('role_rfms') : 0;
        $role->role_property = (in_array(request('role_property'), $valid)) ? request('role_property') : 0;
        $role->role_govtdocs = (in_array(request('role_govtdocs'), $valid)) ? request('role_govtdocs') : 0;
        $role->role_quotations = (in_array(request('role_quotations'), $valid)) ? request('role_quotations') : 0;
        $role->role_contracts = (in_array(request('role_contracts'), $valid)) ? request('role_contracts') : 0;
        $role->role_lpos = (in_array(request('role_lpos'), $valid)) ? request('role_lpos') : 0;
        $role->role_documents = (in_array(request('role_documents'), $valid)) ? request('role_documents') : 0;
        $role->role_rfms_scope = (request('role_rfms_scope') == 'on') ? 'global' : 'own';
        $role->role_vendorrfq = (in_array(request('role_vendorrfq'), $valid)) ? request('role_vendorrfq') : 0;
        $role->role_vendorqtn = (in_array(request('role_vendorqtn'), $valid)) ? request('role_vendorqtn') : 0;
        $role->role_vendorpo = (in_array(request('role_vendorpo'), $valid)) ? request('role_vendorpo') : 0;
        $role->role_vendorinvoice = (in_array(request('role_vendorinvoice'), $valid)) ? request('role_vendorinvoice') : 0;
        $role->role_project_client = (in_array(request('role_project_client'), $valid)) ? request('role_project_client') : 0;
        $role->role_type = 'team';
