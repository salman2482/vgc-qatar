...
<div class="kanban-card show-modal-button reset-card-modal-form js-ajax-ux-request" data-toggle="modal"
    ...
    <div class="x-meta">
        ...
        <div><label class="label label-light-info p-t-3 p-b-3 p-r-8 p-l-8">{{ runtimeMoneyFormat($lead->lead_value) }}</label></div>
        @endif

        <!--telephone-->
        @if(config('system.settings_leads_kanban_telephone') == 'show')
        <span class="wordwrap"><strong>{{ cleanLang(__('lang.telephone')) }}:</strong> {{ $lead->lead_phone ?? '---' }}</span>
        @endif
        <!--date created-->
        ...
        <span><strong>{{ cleanLang(__('lang.contacted')) }}:</strong> {{ runtimeDate($lead->lead_last_contacted ) }}</span>
        @endif
        <!--category-->
        ...
        <span class="wordwrap"><strong>{{ cleanLang(__('lang.email')) }}:</strong> {{ $lead->lead_email ?? '---' }}</span>
        @endif
        <!--source-->
        @if(config('system.settings_leads_kanban_source') == 'show')
        <span class="wordwrap"><strong>{{ cleanLang(__('lang.source')) }}:</strong> {{ $lead->lead_source ?? '---'  }}</span>
        @endif
    </div>
